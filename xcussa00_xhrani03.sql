...
SELECT K.JMENO AS JMENO_KLIENTA, K.PRIJMENI AS PRIJMENI_KLIENTA, P.LOGIN AS LOGIN_PRACOVNIKA
    FROM KLIENT K JOIN OPERACE_S_UCTEM O ON K.ID = O.VLASTNIK
        JOIN PRACOVNIK_BANKY ON O.PROVEDL = PRACOVNIK_BANKY.ID
    WHERE TYP = 'vklad';
...
-- Který pracovník banky provedl nejvíce operací
SELECT P.LOGIN, MAX(*) AS POCET_OPERACI
    FROM PRACOVNIK_BANKY P JOIN OPERACE_S_UCTEM O ON O.PROVEDL = P.ID
    GROUP BY P.LOGIN
